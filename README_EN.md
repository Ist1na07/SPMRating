...
An open-source difficulty rating algorithm for osu!mania **7K** beatmaps. Uses Sigmoid player accuracy model with bisection solving to produce star ratings highly consistent with community consensus.

Trained on **213 beatmaps** (148 Dan + 45 Tournament + 20 Graveyard) via alternating-block Nelder-Mead optimization.
...
- Tournament (45 charts): MAE = 0.2095
- Graveyard (20 charts): MAE = 0.4790
...
where:
- **k = 1.56**: Decay rate (d-D=+1 → accuracy drops to 9%, +2 → 2%)
- **C = 4.0**: Curve shape (matching accuracy = 1/(C+1) = 20%)
- **γ = 0.20**: Target average accuracy score
...
```
SR = D_solved × n_eff/(n_eff + N0)       # Note count normalization (N0=10)
if SR > 9.54: SR = 9.54 + (SR - 9.54)/2  # High SR compression
...
| File | Contents |
|------|----------|
| `tuned_params_sigmoid.json` | 39 total SR parameters (sigmoid aggregation) |
...
std = 0.01 (extremely stable across 213 charts). This validates the physical consistency of the sigmoid model — it's not "random" aggregation but a smooth soft-percentile selection via the accuracy equation.
...
The relation **γ ≈ 1/(C+1)** holds precisely: with C=4.0, γ≈0.2011 ≈ 1/5. This means the target accuracy equals the matching-point accuracy, confirming the model's internal consistency.
...
Parameters were optimized via **alternating-block Nelder-Mead** (5 blocks × 2 rounds, 50→25 iterations):
...
| B3a/b/c | Cross, Release, Inverse feature params | Full | Minimal improvement |

Key finding: **k=1.5 is optimal** (largest single improvement, MAE -3.1%).
